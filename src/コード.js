...
function onOpen() {
  ...
  ui.createMenu('SendControl')
    .addItem('送信制御列の整備（列名統一/型/チェック）', 'setupSendControlColumns')
    .addToUi();

  // ★追加：HubSpot Sync（新GAS領域）
  ui.createMenu('HubSpotSync')
    .addItem('① トークンを ScriptProperties に保存', 'setupHubSpotToken')
    .addSeparator()
    .addItem('② 疎通テスト：CompanyをIDでGET', 'test_read_company_by_id')
    .addToUi();
  
  // Send Control 専用メニュー（新GAS）
  ui.createMenu('Send_Today')
